...
// Computed getter for trending movies
...
onMounted(async () => {
  try {
    await Promise.all([
      ...
      authStore.isAuthenticated ? contentStore.fetchRecommendations() : Promise.resolve()
    ])
    
    trendingMovies.value = contentStore.trendingMovies
    trendingSeries.value = contentStore.trendingSeries
    recommendations.value = Array.isArray(contentStore.recommendations) ? contentStore.recommendations : []
  } catch (e) {
...
<template>
  <div>
      ...
      <div class="container mx-auto px-4 relative z-10">
        <div class="max-w-3xl">
          ...
          <div class="flex flex-wrap gap-4">
            <router-link to="/register" v-if="!authStore.isAuthenticated" class="btn btn-primary px-6 py-3">
              Get Started
            </router-link>
    ...
    <div v-else>
      ...
      <section v-if="authStore.isAuthenticated && recommendations.length > 0" class="mb-12">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-2xl font-bold text-white">Recommended For You</h2>
          <router-link to="/recommendations" class="text-primary-400 hover:text-primary-300 transition-colors text-sm">
            View All
          </router-link>
        </div>
        
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <template v-for="item in recommendations.filter(item => item && typeof item === 'object')" :key="item.id">
            <ContentCard 
              v-if="getContentType(item)"
              :item="item" 
      ...
      <section id="trending" class="mb-12">
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link to="/movies" class="text-primary-400 hover:text-primary-300 transition-colors text-sm">
            View All
          </router-link>
        </div>
      ...
      <section class="mb-12">
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link to="/series" class="text-primary-400 hover:text-primary-300 transition-colors text-sm">
            View All
          </router-link>
        </div>
      ...
      <section class="glass-panel p-8 rounded-2xl my-16">
        <h2 class="text-2xl font-bold text-white mb-8 text-center">AI-Powered Film Experience</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div class="text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary-900/50 text-primary-400 mb-4">
            ...
            <p class="text-gray-400">Our AI learns your preferences to suggest content you'll love.</p>
          </div>
          
          <div class="text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-accent-900/50 text-accent-400 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </div>
            <h3 class="text-lg font-semibold text-white mb-2">Personalized Watchlists</h3>
            <p class="text-gray-400">Create and manage your own watchlists to track what you want to see.</p>
          </div>
          
          <div class="text-center">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-secondary-900/50 text-secondary-400 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
            </div>
            <h3 class="text-lg font-semibold text-white mb-2">Community Ratings</h3>
            <p class="text-gray-400">Rate and review your favorite content and see what others think.</p>
          </div>
        </div>
      </section>
    </div>
